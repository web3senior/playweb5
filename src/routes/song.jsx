...
export default function Profile({ title }) {
  ...
  const handleSongQuery = async () => {
    auth.connectAgent().then(({ web5, userDid }) => {
      let t = toast.loading(`Reading songs...`)
      web5.dwn.records
        .query({
          from: auth.mintDIDnode,
          message: {
                ...
                <div className="card" key={i} title={item.recordId}>
                  <div className="card__body d-flex justify-content-between align-items-center">
                    <div className="d-flex flex-column">
                      ...
                      <p>{item.name}</p>
                      <audio controls>
                        <source src={item.song_file} type="audio/ogg" />
                      </audio>
                    </div>
              ...
              <div className="card__body">
                <ul className="d-flex flex-column" style={{ rowGap: '1rem' }}>
                  ...
                  <li>
                    ...
                    <input type="file" name="song_file" id="song_file" onChange={(e) => changeFile(e)} />
                  </li>
                  <li>
                    <button className="btn" onClick={() => handleNewSong()}>
                      Save
                    </button>
                  </li>
                </ul>
